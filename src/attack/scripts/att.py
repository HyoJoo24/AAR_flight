...
#Expect_action_pub = rospy.Publisher('expect_action'+str(self.drone_id), Action, queue_size=10)
...
def pos_image_cb(msg):
    ...
    if x <= 0:
        ...
        image_failed_cnt = 0
    if image_failed_cnt <= 10 and image_failed_cnt > 0:      # 20有点长
        pass
    else:
        pos_i[0] = x
        pos_i[1] = y
...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
        ...
        # cmd = u.DockingController(pos_info, pos_i, image_center, cam_info)
        # 识别到图像才进行角速度控制
        if pos_i[1] > 0:
