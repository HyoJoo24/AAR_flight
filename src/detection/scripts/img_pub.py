...
def image_callback(data):
    ...
    hue_image = cv2.cvtColor(cv_img, cv2.COLOR_BGR2HSV)
    low_range1 = np.array([0, 43, 46])   #[0, 230, 80]
    high_range1 = np.array([10, 256, 255]) # [5, 256, 200]
    th1 = cv2.inRange(hue_image, low_range1, high_range1)
    low_range2 = np.array([156, 43, 46])   #[0, 230, 80]
    high_range2 = np.array([180, 255, 255]) # [5, 256, 200]
...
def circleDetect_callback(data):
    """Hough Circle detect"""
    ...
    #red domain detect
    hue_image = cv2.cvtColor(cv_img, cv2.COLOR_BGR2HSV)
    low_range1 = np.array([160, 100, 100])   #[0, 230, 80]
    high_range1 = np.array([180, 255, 255]) # [5, 256, 200]
    th1 = cv2.inRange(hue_image, low_range1, high_range1)
    low_range2 = np.array([0, 100, 100])   #[0, 230, 80]
    high_range2 = np.array([10, 255, 255]) # [5, 256, 200]
    th2 = cv2.inRange(hue_image, low_range2, high_range2)
    th = th1 + th2
    mask = th
    ...
    circles=[]
    #circles1 = cv2.HoughCircles(b, cv2.HOUGH_GRADIENT, 1, 200, param1=100, param2=80, minRadius=10, maxRadius=300)

    #red + circle detect
    circles1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 200, param1=50, param2=30, minRadius=1, maxRadius=300)
    ...
    if circles1 is not None:
        ...
        id = 0
        for i in circles :
            cv2.circle(cv_img, (i[0], i[1]), i[2], (0,255,0), 2)#huayuan
            cv2.circle(cv_img, (i[0], i[1]), 2, (0,255,255), 3)#huayuanxin
            #cv2.imshow("img", cv_img)
            bbox = BoundingBox()
            bbox.probability = 1
            bbox.xmin = i[0]-i[2]
...
if __name__ == '__main__':
    ...
    global bridge
    bridge = CvBridge()
    rospy.Subscriber('/camera/left', Image, circleDetect_callback)
