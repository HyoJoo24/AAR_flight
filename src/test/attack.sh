...
roslaunch simulation bs_mavros.launch  mav_id:=${MAVID} use_pix:=${USE_PIX} IP:=${UE4IP} port1:=`expr ${MAVID} \* 2 + 20099` port2:=`expr ${MAVID} \* 2 + 20098` & PID1=$!
sleep 2s
...
# 决策相关
#roslaunch decision drone.launch  drone_id:=${MAVID}  drone_num:=${MAVNUM} & PID3=$!
#sleep 1s

# 打击
#roslaunch attack data_saver.launch & PID4=$!
#sleep 2s
